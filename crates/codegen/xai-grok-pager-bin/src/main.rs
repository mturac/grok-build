...
// Cached ACP replay state (`initialize` + `session/load`), shared with the
// remote WS reconnect path in `xai-grok-pager`'s `acp::leader_bridge`.
use xai_grok_pager::acp::replay::{
    ReplayState, cache_incoming_session_id, cache_outgoing_acp_state,
